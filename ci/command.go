...
func init() {
	...
	stackCmd.Flags().StringVar(&stackSelectFlag, "stack", "", "which stack (name, or `all`; default: all, and the first one for `watch`)")
	stackCmd.Flags().StringVar(&stackEchoModeFlag, "mode", "", "with `echo`: the mode to name in the line")
	stackCmd.Flags().BoolVar(&stackEchoWatchFlag, "watch", false, "with `echo`: use watch selection (the first stack), not batch selection")
	...
	scanCmd.Flags().StringVar(&scanModeFlag, "mode", "", "force the mode (dev|prod) instead of the one ci.policy resolves")
	scanCmd.Flags().StringVar(&scanStackFlag, "stack", "", "with --list-targets: which stack's targets (name, or `all`; default: all)")
	scanCmd.Flags().BoolVar(&scanListTargetsFlag, "list-targets", false, "list the targets an artifact sweep should visit, one per line")
